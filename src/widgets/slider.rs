...
pub struct SliderHandler {
   on_slider_moved: Option<Box<dyn FnMut(f32)>>,
   on_slider_pressed: Option<Box<dyn FnMut(f32)>>,
   on_slider_released: Option<Box<dyn FnMut(f32)>>,
}
...
impl SliderHandler {
   ...
   pub fn on_slider_moved(mut self, cb: impl FnMut(f32) + 'static) -> Self {
   ...
   ///
   /// It allocates memory in heap for the closure.
   pub fn on_slider_pressed(mut self, cb: impl FnMut(f32) + 'static) -> Self {
   ...
   ///
   /// It allocates memory in heap for the closure.
   pub fn on_slider_released(mut self, cb: impl FnMut(f32) + 'static) -> Self {
...
impl ISliderHandler for SliderHandler {
   fn slider_moved(&mut self, v: f32) {
      if let Some(h) = &mut self.on_slider_moved {
         (h)(v)
      }
   }

   fn slider_pressed(&mut self, v: f32) {
      if let Some(h) = &mut self.on_slider_pressed {
         (h)(v)
      }
   }

   fn slider_released(&mut self, v: f32) {
